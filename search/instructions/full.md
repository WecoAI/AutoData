...
- **The eval does not train on your whole returned set.** Because a d8 model only consumes ~r=10 of tokens — far less than 14.37 M docs — the harness draws a **fixed-size representative slice** of the documents you return and trains the d8 model on that slice (3 random seeds, on Modal H100s), then reports the mean validation BPB on `shard_06542`. **Implication:** your job is to make the *whole returned set a uniformly high-quality pool* — do not try to "front-load" the best documents or exploit which slice gets trained; assume any representative subsample of your selection is what's evaluated.
- Metric: `mean(val_bpb)` over the 3 d8 seeds (lower is better); the harness also reports `val_bpb_std`.

Reference random baseline (random-uniform selection over the pool): `val_bpb ≈ 0.9536 ± 0.0004` (the harness logs the step-0 baseline). With only 3 seeds, small improvements are noisy — a gain larger than ≈ `0.001` (≈ 1.3 σ) is far more convincing than ≈ `0.0005`. Prefer robust, transferable rules over marginal current-pool gains.
