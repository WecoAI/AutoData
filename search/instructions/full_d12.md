...
- Eval model: nanochat depth-12 (~286 M parameters).
- **The eval does not train on your whole returned set.** Because a d12 model only consumes ~r=10 of tokens — far less than 14.37 M docs — the harness draws a **fixed-size representative slice** of the documents you return and trains the d12 model on that slice (2 random seeds, on Modal H100s), then reports the mean validation BPB on `shard_06542`. **Implication:** your job is to make the *whole returned set a uniformly high-quality pool* — do not try to "front-load" the best documents or exploit which slice gets trained; assume any representative subsample of your selection is what's evaluated.
- Metric: `mean(val_bpb)` over the 2 d12 seeds (lower is better); the harness also reports `val_bpb_std`.

Reference random baseline (random-uniform selection over the pool): `val_bpb = 0.8431` (measured on this exact harness: 2 seeds, 0.843006 / 0.843118). Seed-to-seed spread is tiny (σ ≈ 0.0001) because both seeds train on the **same** fixed sub-sample — that σ captures training noise only and does **not** measure how much your selection would move under a different draw. So do not read a 0.0002 improvement as real: a gain larger than ≈ `0.001` is convincing, ≈ `0.0005` is suggestive, and anything below that is indistinguishable from sub-sample luck. Prefer robust, transferable rules over marginal current-pool gains.
...
- Larger models: d12 → d24 → d28, ≈ 3–10× more parameters.
...
```text
A robust, pool-adaptive, budget-adaptive document-selection rule that preserves
broad coverage while applying modest quality-aware corrections.
```
